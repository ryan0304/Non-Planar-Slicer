...
**First time in? Press "How to Use?"** (top of the panel, or the **?** key). Six
slides cover the fastest path to a print, the four design steps, the curve
editors, the textures, and what the viewer gives you after generating. Each one
is a short **recording of this app** rather than a diagram — the cursor, the
hover states, the progress on the button and the numbers that change were all
done by the real interface and captured frame by frame, so what you are shown is
...
loop-fabric panel rules, the draft preview's base geometry, the How-to-use
guide's frames, and — because it has been got wrong three times — that
switching back to Design takes *everything* belonging to the G-code viewer off
the shared canvas:
...
viewer/guide.js                 the "How to Use?" onboarding guide and its frame player
viewer/guide/                   its frames: recordings of this app, one directory per slide
