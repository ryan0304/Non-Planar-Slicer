...
|---|---|---|
| The viewer showed "Max Z-rate 5.0 mm/s !" on a live Ender 3 print: a Z-clamped feedrate of 1841.6 mm/min was written as F1842 (5.005 mm/s). Two regression references had carried the same defect on the Trident (25.004 mm/s) all along, and 4-decimal coordinate rounding could push a clamped move to 25.011 even with F rounded down | Every check compared the writer's own full-precision floats, never the numbers actually written into the file -- the only numbers the firmware ever sees | `_clamp_to_emitted_z()` re-clamps on written coordinates, `_feedrate_word()` rounds F down; `test_emitted_feedrate_never_exceeds_z_limit` recomputes the Z-rate from the emitted TEXT on an Ender 3 and a Trident design that drive Z to its ceiling |
| After the custom-filament fix shipped, a real pre-existing custom ("PETG-R3D", saved before `base` was recorded) was sent as `filament=pla` -- PLA's flow ratio and volumetric ceiling on PETG. Every test used a freshly saved custom, which always has a `base` | Tests only exercised data the new code itself wrote; migration of data written by the OLD code was never tried. Found only on a live profile with real saved data | `customFilamentBase()` infers the material from the custom's name/key (PLA only when nothing matches); dev_smoke injects base-less legacy entries |
| A raw request with `print_speed` <= 0 generated ~40k moves at `F0`/negative F and the analyzer reported `issues == []`; `layer_height`/`line_width` <= 0 or tiny hung or 500'd the server; `radius`/`height` <= 0 generated a file. The server clamped none of `layer_height`, `print_speed`, `line_width`, `z_waves`, `pattern_waves`, `base_layers`, `brim` although the UI did | Server clamps looser than UI clamps (CLAUDE.md), plus a guard that could be defeated: nothing ever looked at F itself | `_num()` boundary helper in serve.py; `GcodeWriter._move` rejects F <= 0; analyzer flags an explicit F <= 0 as DANGER (never the unset initial F); `test_serve_input_validation.py` |
| Saving a custom filament in the filament modal and generating failed with "Filament 'custom_x' not found" -- the row had been deliberately hidden ("no backend exists for it"), then re-exposed without one; the dev_smoke assertion guarding it had gone stale and was simply failing | #4 test outlived its assumption -- a failing assertion was read as noise instead of as the feature being broken | Custom filaments resolve to their base material plus explicit overrides client-side (never a `custom_*` key on the wire); dev_smoke asserts the request body |
| The filament modal displayed PLA 205 C / ABS 245 C / TPU 220/45 C as defaults while the server printed 210 / 255 / 230/50 | Two copies of one table with nothing tying them together | Client table re-synced to `orca.py`'s BASIC_FILAMENTS, comment says both must change together |
| The G-code viewer ignored G90/G91 and G92, so every Bambu/Creality file this app generates previewed its end lift as a plunge to Z=10 through the part | Viewer parser not mirroring analyze.py | parseGcode mirrors analyze.py's G90/G91/G92/M82 handling; dev_smoke feeds it the real end sequence and an M82 + G92 E0 file |
| A loaded `.trident` file (or resumed session) with `layer_height: 0` reached the preview math unclamped (`turns = height / 0`) | #2 applied to validation: select fields were repaired on load, numeric fields never were | `repairNumericFields()` next to every `repairSelectBackedFields()` call |
| At 390 px wide the top-bar actions (undo/redo/load/history) were pushed off-screen and could not be scrolled to; the Point Edit modal overflowed | Nothing measured layout below tablet width | Flex shrink/ellipsis on the title; Point Edit modal stacks under 560 px |
